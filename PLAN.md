...
5. **Manual fallback + GPS checkout.** The doctor can always set status by hand (check in/out, on call, "away until…") for places without a beacon, dead phone batteries, or corrections. Manual entries override beacon-derived state until the next beacon event. A manual check-in records the phone's **GPS position as an anchor** (also teaching the place its coordinates on first use) and arms a **geofence**: when the doctor moves more than `checkout_radius_meters` away (default **500 m**, configurable in `app_config`), the phone checks them out automatically and confirms with a local notification — so a beacon-less check-in can't stay open forever.
